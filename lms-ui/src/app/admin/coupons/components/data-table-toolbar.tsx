...
const DataTableToolbar = <TData,>({
  ...
  const isFiltered = searchValue !== '' || statusFilter.length > 0 || discountTypeFilter.length > 0
      ...
      <div className='flex flex-1 flex-col-reverse items-start gap-y-2 sm:flex-row sm:items-center sm:space-x-2'>
        ...
        <div className='flex gap-x-2'>
          <DataTableFacetedFilter
            title='Trạng thái'
            options={FILTER_OPTIONS.COUPON_STATUS}
          ...
          <DataTableFacetedFilter
            title='Loại giảm giá'
            options={FILTER_OPTIONS.COUPON_DISCOUNT_TYPE}
            selectedValues={discountTypeFilter}
            onSelectionChange={onDiscountTypeFilterChange}
            disabled={isLoading}
          />
        </div>
        {isFiltered && (
        ...
        {hasSelectedRows && (
          <>
            <span className='text-muted-foreground text-sm'>
              Đã chọn {selectedRows.length} / {table.getFilteredRowModel().rows.length} hàng
            </span>
            <Button
              variant='outline'
              size='sm'
            ...
            >
              ...
              Xóa {selectedRows.length} {selectedRows.length === 1 ? 'mã' : 'mã'}
            </Button>
          </>
        )}
