...
const CouponsTable = () => {
  ...
  const queryParams = useMemo(
    ...
    [currentPage, pageSize, debouncedSearch, filters.status, filters.discountType, filters.sortBy, filters.sortOrder]
  )

  const { data: couponsData, isLoading } = useCoupons(queryParams)

  // Đặt lại về trang đầu khi bộ lọc thay đổi (tối ưu dependencies)
  useEffect(() => {
    setCurrentPage(PAGINATION_CONSTANTS.DEFAULT_PAGE)
  }, [debouncedSearch, filters.status, filters.discountType, filters.sortBy, filters.sortOrder])
  ...
  const coupons = useMemo(() => couponsData?.coupons || [], [couponsData?.coupons])
