...
const VerifyEmailPage = () => {
  ...
  // Mutation xác minh email với TanStack Query
  const verifyEmailMutation = useVerifyEmail()

  // Thực hiện xác minh khi component được mount
  useEffect(() => {
    if (!token) {
      setState('error')
    ...
    // Kích hoạt xác minh kèm callback xử lý kết quả
    verifyEmailMutation.mutate(
      { token },
      {
        onSuccess: (response: ApiResponse) => {
          ...
          // Tự động chuyển hướng khi thành công sau 3 giây
          setTimeout(() => {
          ...
          <CardContent className='space-y-6'>
            ...
            {currentState === 'success' && (
              <div className='space-y-4 text-center'>
                ...
                <p className='text-muted-foreground text-sm'>Đang chuyển bạn đến trang đăng nhập sau vài giây...</p>
