...
interface CircularProgressProps {
  value: number; // Phan tram tu 0-100
  size?: "sm" | "md" | "lg" | "xl";
  thickness?: number;
  color?: "primary" | "blue" | "orange" | "green" | "red" | "gray";
  showValue?: boolean;
  className?: string;
...
export function CircularProgress({
  ...
  const { stroke, background, text } = colorMap[color] ?? colorMap.primary;
  ...
  return (
    <div className={cn("relative inline-flex items-center justify-center", className)}>
      <svg width={svgSize} height={svgSize} className="transform -rotate-90" viewBox={`0 0 ${svgSize} ${svgSize}`}>
        {/* Vong nen */}
        <circle
          cx={svgSize / 2}
          cy={svgSize / 2}
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {children ? (
          ...
          <span className={cn("font-semibold", fontSize, text)}>{Math.round(value)}%</span>
        ) : null}
      </div>
    </div>
  );
