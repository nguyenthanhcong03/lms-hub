...
export function useUpdateCourse() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      toast.error(error?.message || 'Không cập nhật được khóa học')
    }
  })
}
...
export function useDeleteCourse() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      toast.error(error?.message || 'Không xóa được khóa học')
    }
  })
}
...
export function useBulkDeleteCourses() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedCourseIds) => {
      console.log('Courses được xóa thành công!')
