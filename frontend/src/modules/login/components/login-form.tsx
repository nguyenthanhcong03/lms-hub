...
import { setLocalUserData } from "@/shared/helpers/storage";
import { loginAuth } from "@/shared/services/auth";
import { getAxiosErrorMessage } from "@/utils";

import { authSchema, AuthSchema } from "@/utils/validation";
...
const LoginForm = ({ returnUrl }: { returnUrl: string }) => {
  ...
  const defaultValues = {
    email: "phanthanhtincr7@gmail.com",
    password: "020101",
  };
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      ...
      if (res?.data) {
        ...
        toast.error(res.message || "Đăng nhập tài khoản thất bại!");
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 md:space-y-6" action="#">
      ...
      <div className="my-4 flex items-center gap-4">
        ...
        <p className="text-center text-sm text-slate-800">or</p>
        <hr className="w-full border-slate-300" />
      </div>
